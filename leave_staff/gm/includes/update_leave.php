<?php
	if(isset($_POST['update'])){
		...
		$gmStatus = $row2['gmStatus'];
		$empID = $row2['empID'];
		$numberDays = $row2['numberDays'];

		if($status==1 AND $gmStatus==0){
			$sql="UPDATE tblleaves SET gmRemark='$remark', gmStatus='$status', gmDate='$date', leaveStatus=1 WHERE leaveID='$did'";
		...
		}elseif($status==2 AND $gmStatus==0){
			$sql="UPDATE tblleaves SET gmRemark='$remark', gmStatus='$status', gmDate='$date', leaveStatus=2 WHERE leaveID='$did'";
			$query= mysqli_query($conn, $sql) or die(mysqli_error($conn));
			if($query){
